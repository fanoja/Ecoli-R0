## Alternative model with tau = 1/gamma, mean infection aka colonisation time ##
...
import cluster.scripts.load_data
importlib.reload(cluster.scripts.load_data)
...
class CustomPrior_beta(elfi.Distribution):
    def rvs(tau, min_R0, max_R0, size=1, random_state=None):
        #u = scipy.stats.uniform.rvs(loc=0, scale=1, size=size, random_state=random_state)
        len_tau = tau.shape[0]
        beta = np.zeros((len_tau,))
...
bsi_obs_data = bsi_obs = get_incidence_data("data/NORM_incidence.csv", clade = clade, is_prop = is_prop, n_incidence_pop = pop_size, partial_time = False)

# Choose the outbreak period:
...
if not reparam:
    if clade == "A":
        ...
        #tau = elfi.Prior(scipy.stats.norm, 30/7, 10, model = m)
        tau = elfi.Prior(scipy.stats.lognorm, 1.5, 1, model = m) # COMBAT study: mean col time 30 days, up to 12 months.
        ...
        #gamma = elfi.Prior(scipy.stats.norm, 1/(30/7), 0.1, model = m)
        tau = elfi.Prior(scipy.stats.lognorm, 1.5, 1, model = m)
    
    beta = elfi.Prior(CustomPrior_beta, tau, 1, 5, model = m) 
...
if reparam:
    ...
    SIRsim = elfi.Operation(SIR_tau, beta, tau, nt, N, I0, reparam_node, is_prop, model = m)
